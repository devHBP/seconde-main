    ...
    <x-slot name="header">
        <div class="dashboard-header">
            ...
            <div class="header-right-button">
                <a href="{{ route('gestion.get.users', $enseigne->slug) }}" class="">
                    Retour au dashboard
                </a>
            </div>
        </div>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 ">
            <div class=" p-6 create-or-modify">
                ...
                <form action="{{ isset($user) ? route('gestion.modify.user', ['enseigne_slug' => $enseigne->slug, 'user_id' => $user->id]) : route('gestion.store.user', $enseigne->slug) }}" method="POST">
                    @csrf
                    @if(isset($user))
                        @method('PUT')
                    @endif
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <a href="{{ route('gestion.get.users', $enseigne->slug) }}" class="cancel">Annuler</a>
                    </div>
                </form>
            </div>
        </div>
